...
在早期網頁的標準相對陽春的時代，這樣子處理是完全沒有問題的，我們甚至不需要引入外部 js 及 css ，直接寫在 `html` 中就好了。

但是隨著  網頁技術越趨成熟，越來越多的功能被加入，前端的規模變大，原本不明顯的問題慢慢的浮現。

### 沒有模組的 JavaScript

最初在設計 JavaScript 時， 其目標並不是程式語言而是以腳本語言為方向所發明的。因此沒有模組相關的語意概念，而日益龐大的代碼量使其出現了許多的問題。
